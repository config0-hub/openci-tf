...
TEST2_REGIONS = {
    "test2-eu-west-1": ("eu-west-1", "10.42.0.0/16"),
    "test2-us-east-1": ("us-east-1", "10.43.0.0/16"),
}
EXPECTED_ROOTS = [
    ...
    "eu-west-1/01-vpc",
    "eu-west-1/02-ec2",
    "test2-eu-west-1/01-vpc",
    "test2-us-east-1/01-vpc",
]
ALLOWED_ACCOUNT = "REPLACE_MAIN_ACCOUNT"
TEST2_ACCOUNT = "REPLACE_SECONDARY_ACCOUNT"
MAIN_ALIAS = "REPLACE_MAIN_ALIAS"
TEST2_ALIAS = "REPLACE_SECONDARY_ALIAS"
TEST2_STATE_BUCKET = "openci-tf-state-REPLACE_SECONDARY_ACCOUNT"
REPO_SLUG = "<REPO_ORG>/<REPO_NAME>"
STATE_BUCKET = "openci-tf-state-REPLACE_MAIN_ACCOUNT"
...
OPENCI_TF_FOLDER_CONFIG_KEYS = frozenset(
    {"version", "account_alias", "execution_target", "tf_runtime", "timeout", "extra_flags"}
)
...
WORKLOAD_STATE_SUFFIX = ".tfstate"  # legacy combined key reused for 02-ec2
...
class TerraformContractTests(unittest.TestCase):
    ...
    def test_one_public_subnet_per_vpc(self) -> None:
        for region in EXPECTED_REGIONS:
            ...
            self.assertNotIn("for_each = toset(data.aws_availability_zones.available.names)", content)
            self.assertIn("selected_availability_zone = sort(data.aws_availability_zones.available.names)[0]", content)
            self.assertIn("availability_zone       = local.selected_availability_zone", content)
    ...
    def test_workload_roots_read_vpc_remote_state(self) -> None:
        for region in EXPECTED_REGIONS:
            ...
            self.assertIn("vpc_id      = data.terraform_remote_state.vpc.outputs.vpc_id", content)
    ...
    def test_account_guard_present(self) -> None:
        ...
        self.assertEqual(content.count(f'allowed_account = "{TEST2_ACCOUNT}"'), 2)
    ...
    def test_account_guard_wired_to_all_resource_families(self) -> None:
        for region in EXPECTED_REGIONS:
            ...
            vpc_section = vpc_content.split('resource "aws_vpc" "main"', 1)[1].split("resource ", 1)[0]
            ...
            for marker in (
                ...
                self.assertIn("depends_on = [terraform_data.account_guard]", section, marker)
    ...
    def test_disposable_tags_documented(self) -> None:
        content = all_terraform_text()
        for tag in ("ManagedBy", "Project", "Environment", "Region", "ExpiresOn", "Owner", "Disposable"):
            self.assertIn(tag, content)
        self.assertIn('approved_test_date = "2026-08-06"', content)
        self.assertIn('ExpiresOn   = "2026-08-13"', content)

    def test_backend_block_complete_per_root(self) -> None:
        for root in EXPECTED_ROOTS:
            fields = parse_backend_block(root)
            if root.startswith("test2-"):
                for key in ("region", "encrypt"):
                    self.assertEqual(fields.get(key), BACKEND_REQUIRED_FIELDS[key], f"{root} backend {key}")
                self.assertEqual(fields.get("bucket"), TEST2_STATE_BUCKET, root)
    ...
    def test_test2_vpc_roots_only_in_expected_regions(self) -> None:
        for folder, (region, cidr) in TEST2_REGIONS.items():
            content = read_root_files(f"{folder}/01-vpc")
            self.assertIn(f'aws_region      = "{region}"', content)
            self.assertIn(f'vpc_cidr        = "{cidr}"', content)
            self.assertNotIn("ap-northeast-1", content)
        self.assertNotIn("test2-ap-northeast", all_terraform_text())

    def test_backend_schema_allows_plain_init_without_aws(self) -> None:
        """Prove offline init has bucket/key/region without contacting AWS."""
        for root in EXPECTED_ROOTS:
            fields = parse_backend_block(root)
            missing = [name for name in ("bucket", "key", "region") if not fields.get(name)]
...
class OpenciTfConfigTests(unittest.TestCase):
    def test_folder_configs_exist_and_safe(self) -> None:
        ...
        for path in configs:
            ...
            if "test2-" in str(path):
                self.assertIn(f"account_alias: {TEST2_ALIAS}", text)
            else:
    ...
    def test_openci_tf_safe_lane_matches_outer_state_contract(self) -> None:
        ...
        self.assertEqual(OPENCI_TF_SAFE_ACTIONS, frozenset({"plan", "drift", "validate", "report"}))
...
class RepositoryDocsTests(unittest.TestCase):
    ...
    def test_justfile_requires_confirmations_without_backend_injection(self) -> None:
        ...
        self.assertIn(TEST2_ACCOUNT, justfile)
        self.assertIn(REPO_SLUG, justfile)
        self.assertIn("confirm-test-account", justfile)
        self.assertIn("confirm-test2-account", justfile)
