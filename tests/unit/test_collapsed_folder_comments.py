...
"""Focused tests for collapsed per-folder PR comments and unified report layout."""
...
def _assert_report_folder_shell(body: str, *, folder: str) -> None:
    assert body.startswith("<details>\n<summary>")
    assert ' open' not in body.split("</summary>", 1)[0]
    assert f"{folder} · Drift" in body
    assert "Security" in body.split("</summary>", 1)[0]
...
@pytest.mark.parametrize(
    "outcome",
    [
        _outcome(status="in_progress", reply="Run already in progress (exec other)."),
        _outcome(status="infrastructure_error", error="engine failed"),
        _outcome(credential_expired=True),
        _outcome(succeeded=False, error="step failed"),
        _outcome(),
    ],
)
def test_folder_comment_terminal_paths_use_report_summary_line(outcome):
    rendered = folder_comment("infra/a", outcome, _artifacts(), action="plan", commit_hash=_FULL_SHA)
    _assert_report_folder_shell(rendered, folder="infra/a")
...
def test_folder_comment_escapes_html_in_summary():
    ...
    assert "<summary>infra/&lt;bad&gt; · Drift" in rendered
...
def test_pending_plan_comment_uses_report_pending_summary_line():
    rendered = pending_plan_comment("infra/a", _ACCOUNT, _FULL_SHA, "plan")
    assert rendered.startswith("<details>")
    assert "infra/a · Drift ⏳ · Security ⏳" in rendered
    assert "## openci-tf plan" in rendered


def test_pending_summary_uses_report_columns_without_account():
    ...
    assert "| `infra/a` | ⏳ | ⏳ | — |" in rendered
    assert "| `infra/b` | ⏳ | ⏳ | — |" in rendered


def test_summary_table_uses_report_columns_without_account():
    rendered = summary(
        [
    ...
    assert "| Folder | Drift | Security | Cost |" in rendered
    assert "| Account |" not in rendered
    assert "| `infra/good` | ✅ |" in rendered
    assert "| `infra/bad` | ❌ |" in rendered
...
def test_metadata_section_omits_removed_after_acknowledgement_text():
    block = metadata_section(
        action="apply",
    ...
    assert "<summary>Metadata</summary>" in block
    assert "removed after acknowledgement" not in block
...
def test_render_placeholder_appends_metadata_with_account(monkeypatch):
    ...
    assert f"- account: `{_ACCOUNT}`" in body
    assert "infra/a · Drift ⏳ · Security ⏳" in body
