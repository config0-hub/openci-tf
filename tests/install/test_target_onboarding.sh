...
assert_account_row_json() { # <desc> <expected-alias> <expected-max-ttl>
	...
	if python3 - "$SSM_STORE/account-row" "$expected_alias" "$expected_max_ttl" <<'PY'; then
import json
import sys
...
assert data["account_id"] == {"S": "222222222222"}
assert data["role_name"] == {"S": "openci-tf-executor-readonly"}
assert data["external_id"] == {"S": "openci-tf-b3b391e562bd71b6"}
if expected_max_ttl:
...
expect_rc "derive_external_id rejects short hub id" 1 "$SCRIPTS/derive_external_id.sh" 123 222222222222
expect_rc "derive_external_id rejects target letters" 1 "$SCRIPTS/derive_external_id.sh" 111111111111 abcdefghijkl
ID1="$($SCRIPTS/derive_external_id.sh 111111111111 222222222222)"
ID2="$($SCRIPTS/derive_external_id.sh 111111111111 222222222222)"
ID3="$($SCRIPTS/derive_external_id.sh 333333333333 222222222222)"
[[ "$ID1" = "$ID2" ]] && echo "PASS derive_external_id is deterministic" || {
	echo "FAIL derive_external_id mismatch"
	FAILURES=$((FAILURES + 1))
}
[[ "$ID1" != "$ID3" ]] && echo "PASS derive_external_id varies by hub account" || {
	echo "FAIL derive_external_id ignores hub account"
	FAILURES=$((FAILURES + 1))
}
[[ "$ID1" = "openci-tf-b3b391e562bd71b6" ]] && echo "PASS derive_external_id known vector" || {
	echo "FAIL derive_external_id vector got $ID1"
	FAILURES=$((FAILURES + 1))
}
[[ "$ID1" =~ ^openci-tf-[0-9a-f]{16}$ ]] && echo "PASS derive_external_id format" || {
	echo "FAIL derive_external_id format"
	FAILURES=$((FAILURES + 1))
}

# bucket_from_s3_arn
expect_rc "bucket_from_s3_arn rejects garbage" 1 "$SCRIPTS/bucket_from_s3_arn.sh" not-an-arn
BUCKET_NAME="$("$SCRIPTS/bucket_from_s3_arn.sh" arn:aws:s3:::openci-tf-state-222222222222)"
[[ "$BUCKET_NAME" = "openci-tf-state-222222222222" ]] && echo "PASS bucket_from_s3_arn extracts name" || {
	echo "FAIL bucket_from_s3_arn"
	FAILURES=$((FAILURES + 1))
}

# append_target_account_id: unique append + jq validation
write_aws_mock 111111111111 yes
write_just_mock
printf '%s' '["111111111111"]' >"$SSM_STORE/target_account_ids"
(cd "$REPO" && "$SCRIPTS/append_target_account_id.sh" 222222222222) >/dev/null
UPDATED="$(jq -c . "$SSM_STORE/target_account_ids")"
[[ "$UPDATED" = '["111111111111","222222222222"]' ]] && echo "PASS append_target_account_id appends uniquely" || {
	echo "FAIL append got $UPDATED"
	FAILURES=$((FAILURES + 1))
}
(cd "$REPO" && "$SCRIPTS/append_target_account_id.sh" 222222222222) >/dev/null
[[ "$(jq -c . "$SSM_STORE/target_account_ids")" = "$UPDATED" ]] && echo "PASS append_target_account_id is idempotent" || {
	echo "FAIL append not idempotent"
	FAILURES=$((FAILURES + 1))
}
printf '%s' 'not-json' >"$SSM_STORE/target_account_ids"
expect_rc "append_target_account_id rejects malformed JSON" 1 bash -c "cd '$REPO' && '$SCRIPTS/append_target_account_id.sh' 222222222222"
...
[[ "$RESOLVED" = "custom-target-bucket" ]] && echo "PASS target_connect_state_bucket reads SSM ARN" || {
	echo "FAIL resolved=$RESOLVED"
	FAILURES=$((FAILURES + 1))
}
rm -f "$SSM_STORE/target_state_bucket_arn"
DEFAULT="$(cd "$REPO" && "$SCRIPTS/target_connect_state_bucket.sh")"
[[ "$DEFAULT" = "openci-tf-state-111111111111" ]] && echo "PASS target_connect_state_bucket defaults to conventional name" || {
	echo "FAIL default=$DEFAULT"
	FAILURES=$((FAILURES + 1))
}
...
(cd "$REPO" && "$SCRIPTS/register_account.sh" --alias "$ALIAS_PAYLOAD" --account-id 222222222222 --role-name openci-tf-executor-readonly --max-ttl 0900) >/dev/null
...
rm -rf "$MOCK_CALLS"/* "$SSM_STORE"/*
write_aws_mock 222222222222 yes
write_just_mock
(cd "$REPO" && "$SCRIPTS/target_onboard.sh" --hub-account-id 111111111111) >/dev/null
...
[[ "$(cat "$SSM_STORE/hub_lambda_exec_role_arn")" = "arn:aws:iam::111111111111:role/openci-tf-hub-lambda-exec" ]] && echo "PASS target_onboard hub role arn value" || {
	echo "FAIL hub role arn"
	FAILURES=$((FAILURES + 1))
}
[[ "$(cat "$SSM_STORE/target_state_bucket_arn")" = "arn:aws:s3:::openci-tf-state-222222222222" ]] && echo "PASS target_onboard bucket arn value" || {
	echo "FAIL bucket arn"
	FAILURES=$((FAILURES + 1))
}
...
rm -rf "$MOCK_CALLS"/* "$SSM_STORE"/*
write_aws_mock 222222222222 no
write_just_mock
expect_rc "target_onboard fails when bucket missing" 1 bash -c "cd '$REPO' && '$SCRIPTS/target_onboard.sh' --hub-account-id 111111111111"
...
rm -rf "$MOCK_CALLS"/* "$SSM_STORE"/*
write_aws_mock 222222222222 denied
write_just_mock
expect_rc "target_onboard fails on denied bucket probe" 2 bash -c "cd '$REPO' && '$SCRIPTS/target_onboard.sh' --hub-account-id 111111111111"
...
write_just_mock
printf '%s' '["111111111111"]' >"$SSM_STORE/target_account_ids"
printf '%s' 'stale-external-id' >"$SSM_STORE/account-row"
(cd "$REPO" && "$SCRIPTS/register_target.sh" --alias platform-target --account-id 222222222222) >/dev/null
...
[[ -e "$MOCK_CALLS/dynamodb-put" ]] && echo "PASS register_target registers account row" || {
	echo "FAIL register_target missing dynamodb put"
	FAILURES=$((FAILURES + 1))
}
if grep -Fq 'openci-tf-b3b391e562bd71b6' "$MOCK_CALLS/dynamodb-put-args"; then
	echo "PASS register_target stores derived external_id"
else
	echo "FAIL register_target missing derived external_id"
	FAILURES=$((FAILURES + 1))
fi
if grep -Fq 'openci-tf-b3b391e562bd71b6' "$SSM_STORE/account-row" && ! grep -Fq 'stale-external-id' "$SSM_STORE/account-row"; then
	echo "PASS register_target replaces stale external_id"
else
	echo "FAIL register_target did not replace stale external_id"
	FAILURES=$((FAILURES + 1))
fi
[[ "$(jq -c . "$SSM_STORE/target_account_ids")" = '["111111111111","222222222222"]' ]] && echo "PASS register_target appends target_account_ids" || {
	echo "FAIL register_target ids"
	FAILURES=$((FAILURES + 1))
}
SEQUENCE="$(tr '\n' ' ' <"$MOCK_CALLS/sequence" | sed 's/ $//')"
[[ "$SEQUENCE" = "append-target-id deploy register-alias" ]] && echo "PASS register_target operation order" || {
	echo "FAIL register_target order got '$SEQUENCE'"
	FAILURES=$((FAILURES + 1))
}
...
write_failing_deploy_just_mock
printf '%s' '["111111111111"]' >"$SSM_STORE/target_account_ids"
expect_rc "register_target fails loud when deploy fails" 1 bash -c "cd '$REPO' && '$SCRIPTS/register_target.sh' --alias platform-target --account-id 222222222222"
assert_no_call "register_target deploy failure skips alias row" dynamodb-put
[[ "$(jq -c . "$SSM_STORE/target_account_ids")" = '["111111111111","222222222222"]' ]] && echo "PASS register_target deploy failure still appends target id" || {
	echo "FAIL register_target deploy failure ids"
	FAILURES=$((FAILURES + 1))
}
