#!/usr/bin/env bash
# poweruser_needs_destroy decision semantics for uninstall.
set -euo pipefail
REPO="$(cd "$(dirname "$0")/../../" && pwd)"
cd "$REPO"

failures=0
decision_script="$REPO/scripts/poweruser_needs_destroy.sh"
...
test_decision "policy access denied abort" 254 "$nosuch_role" 254 \
  "An error occurred (AccessDenied) when calling the GetPolicy operation" 2
test_decision "role generic 404 abort" 1 "404 Not Found from intermediary endpoint" 254 "$nosuch_policy" 2

if [ "$failures" -gt 0 ]; then
  echo "poweruser_needs_destroy tests: ${failures} failure(s)"
  exit 1
fi
echo "poweruser_needs_destroy tests: all passed"
