...
case "$ACTION" in
  create|destroy) ;;
  *) echo "ERROR: --action must be create or destroy" >&2; exit 1 ;;
esac

case "$ROLE_KIND" in
  readonly|poweruser) ;;
  *) echo "ERROR: --role must be readonly or poweruser" >&2; exit 1 ;;
esac
...
if [ "$ROLE_KIND" = "readonly" ] && [ "$HUB_ACCOUNT_ID" = "$TARGET_ACCOUNT_ID" ]; then
  ...
  echo "ERROR: Hub readonly is owned by hub-setup deploy (just deploy). See docs/MIGRATION_EXECUTOR_ROLES.md." >&2
  exit 1
fi
