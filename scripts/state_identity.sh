...
#     (legacy pre-S3-lockfile installs; apply/destroy removes it)
...
#
# Usage: state_identity.sh <tfstate> <expected_bucket>

STATE="${1:?Usage: state_identity.sh <tfstate> <bucket>}"
BUCKET="${2:?expected bucket required}"

if ! jq -e --arg b "$BUCKET" '
  ...
  # legacy managed lock table optional; when present it must be at address "locks"
  and ($tables | length <= 1)
  and (($tables | length == 0) or ($tables[0].name == "locks"))
...
' "$STATE" >/dev/null; then
  echo "ERROR: local state ${STATE} is not exactly the openci-tf bootstrap stack for ${BUCKET}; refusing to use it" >&2
  exit 1
fi
