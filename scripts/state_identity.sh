...
#     tracking <lock_table> (absence allowed: partial-apply window)
#   - every managed resource must be root-module and on the bootstrap-stack
#     allowlist (bucket, versioning, sse, public-access-block, lock table) —
#     any unrelated managed resource, duplicate, or module resource is rejected
...
#
# Usage: state_identity.sh <tfstate> <expected_bucket> <expected_lock_table>

STATE="${1:?Usage: state_identity.sh <tfstate> <bucket> <lock_table>}"
BUCKET="${2:?expected bucket required}"
LOCK_TABLE="${3:?expected lock table required}"

if ! jq -e --arg b "$BUCKET" --arg t "$LOCK_TABLE" '
  ...
  # managed lock table optional; when present: address "locks", expected name
  and ($tables | length <= 1)
  and (($tables | length == 0)
...
' "$STATE" >/dev/null; then
  echo "ERROR: local state ${STATE} is not exactly the openci-tf bootstrap stack for ${BUCKET}/${LOCK_TABLE}; refusing to use it" >&2
  exit 1
fi
