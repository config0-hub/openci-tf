#!/usr/bin/env bash
# Tri-state IAM role existence probe for optional/legacy roles.
# Exit 0 = present, 1 = exact not-found (NoSuchEntity), 2 = indeterminate.
set -euo pipefail

ROLE_NAME="${1:?Usage: role_probe.sh <role_name>}"

err="$(mktemp)"
set +e
aws iam get-role --role-name "$ROLE_NAME" >/dev/null 2>"$err"
probe_rc=$?
set -e
...
# stderr line matching the complete GetRole NoSuchEntity signature are treated
...
NOSUCH_LINE_RE='^(aws: \[ERROR\]: )?An error occurred \(NoSuchEntity\) when calling the GetRole operation: .+$'
