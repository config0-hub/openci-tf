#!/usr/bin/env bash
# Decide whether target-connect readonly destroy must run during uninstall.
...
ROLE="${PROJECT}-executor-readonly"
...
if [ "$role_rc" -eq 2 ] || [ "$boundary_rc" -eq 2 ]; then
  echo "ERROR: indeterminate readonly footprint probe (role rc=${role_rc}, boundary rc=${boundary_rc})" >&2
  exit 2
fi
