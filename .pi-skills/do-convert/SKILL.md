---
name: do-convert
description: 'Pi converter: `/do-convert --herdr <plan.md>` idempotently decomposes an approved big plan into Herdr `plan.md` + `route.yaml`, runs conversion-time visual review and validation, and returns `DESIGN_REQUIRED` instead of inventing unresolved architecture.'
argument-hint: --herdr <plan.md> [--out <converted-run-dir>] [--non-interactive]
---
...
Pi conversion front door. It converts an approved big plan into a runnable Herdr run directory only when invoked with `--herdr`.

Use the shared conversion contract below with command name `do-convert`.
