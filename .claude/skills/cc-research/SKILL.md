...
1. Slugify the title. Determine version by globbing `~/project/repos/jiffy-rewrite-2026-ops/mkdocs/docs/work-log/<YYYY-MM-DD>/<slug>/research/v*/`.
...
**Default (no `--team`):**
- For each aspect, spawn an `Explore` subagent with a focused question, instructed to write its findings to `~/project/repos/jiffy-rewrite-2026-ops/mkdocs/docs/work-log/<YYYY-MM-DD>/<slug>/research/v<N>/sections/<aspect>.md` and return a one-paragraph summary.
...
When all Explore agents have returned, **dispatch a synthesizer subagent** with the section-file paths. The synthesizer reads each section file and writes **two consolidated outputs** into `~/project/repos/jiffy-rewrite-2026-ops/mkdocs/docs/work-log/<YYYY-MM-DD>/<slug>/research/v<N>/`:
...
- Use the dark style from `~/project/repos/jiffy-rewrite-2026-ops/mkdocs/docs/diagrams/architecture/v3.html` (`<style>` block) if it exists; otherwise the default style block from the `/design-doc` skill.
...
1. **Dispatch a nav-sync subagent** to update `~/project/repos/jiffy-rewrite-2026-ops/mkdocs/mkdocs.yml` under `- Logs:`.
2. If `--team`: SendMessage `STOP_PULSE` to team-pulse if it was running.
3. Ensure the work-log static server is up (idempotent — no-op if already running): `bash ~/project/repos/jiffy-rewrite-2026-ops/tools/serve-worklog.sh up`
4. Tell the user:
   ```
   Research saved.
